	...
	state select_best_move(const board& b) const {
		...
		for (state* move = after; move != after + 4; move++) {
			if (move->assign(b)) {
				/*
				 * before state
				 * first call before_state() to get a before_state board
				 * then iterate through all possible empty space with value 1 & 2
				 * and sum up the estimated value
				 * then compare the estimated value with the best one
				 */
				/*float value = 0;
				board bs = move->after_state();
				for (int space=0; space < 16; space++){
					if (bs.at(space) == 0){
						board tmp = bs;
						tmp.set(space, 1);
						value += estimate(tmp) * 0.9;
						tmp.set(space, 2);
						value += estimate(tmp) * 0.1;
					}
				}*/
				//move->set_value(move->reward() + value);
				move->set_value(move->reward() + estimate(move->after_state())); //after state
...
void display(const board& b, int epoch, float high, float score){
	system("clear");
	cout << "Epoch :" << std::to_string(epoch) << endl;
	cout << "--------------------" << endl;
	for (int i = 0; i<16; ++i){
		if (i!=0 && (i+1)%4==0)
			cout << setw(5) << ((1 << b.at(i)) & -2u) << endl;
		else
			cout << setw(5) << ((1 << b.at(i)) & -2u);
	}
	cout << "--------------------" << endl;
	cout << "Highest :" << std::to_string(high) << "\tCurrect Score :" << std::to_string(score) << endl;
}
...
int main(int argc, const char* argv[]) {
	...
	for (size_t n = 1; n <= total; n++) {
		...
		mean = tdl.make_statistic(n, b, score);
	...
	long train_time = end_time - start_time;
	int hr = train_time / 3600;
	int min = (train_time - hr * 3600) / 60;
	int sec = (train_time - hr*3600 - min*60);
	cout << "Total training time :" << std::to_string(hr) << " hrs " \
		<< std::to_string(min) << " mins " << std::to_string(sec) << " secs" << endl;


	ofstream file;
	file.open("as_mean.txt", ios::out);
	if (file.is_open()){
		for (float mean : mean_list){
			file << mean << "\t";
		}
		file.flush();
		file.close();
